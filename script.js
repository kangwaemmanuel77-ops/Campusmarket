// ==========================
// CAMPUS MARKET JS (FIXED FOR DEPLOYMENT)
// ==========================
...
function initSupabase() {
    if (window.supabase && window.supabase.createClient) {
        ...
        startApp(); // ONLY run app after supabase is ready
    } else {
...
// ==========================
function startApp() {
    // FIXED: Using split to isolate the specific filename for robust cross-platform routing
    ...
    // FIXED: Checks for clean root URLs commonly generated by cloud platform targets
    if (page === "index.html" || page === "" || window.location.pathname === "/") {
        loadItems();
    }

    // ==========================
    // MARKETPLACE PAGE SYSTEM (SEARCH & FILTERS)
    // ==========================
    ...
    // ==========================
    // ITEM DETAILS PAGE (WITH WHATSAPP CHAT)
    // ==========================
    async function loadItemPage() {
        ...
        box.innerHTML = `
            <img src="${data.image_url}" style="width:100%;max-width:400px;" alt="${data.title}">
            <h2>${data.title}</h2>
            ...
            <div style="margin-top: 25px;">
                <a href="https://wa.me/${cleanNumber}?text=Hi,%20I'm%20interested%20in%20your%20listing%20'${encodeURIComponent(data.title)}'%20on%20Campus%20Market." 
                   target="_blank" 
                   style="display: inline-flex; align-items: center; gap: 8px; background: #25D366; color: white; padding: 12px 20px; text-decoration: none; border-radius: 6px; font-weight: 600; font-family: 'Poppins', sans-serif;">
                    <i class="fa-brands fa-whatsapp" style="font-size: 1.2rem;"></i> Contact Seller via WhatsApp
                </a>
            </div>
        `;
    }
    ...
    // ==========================
    // SELL PAGE (PROTECTED & USER-LINKED)
    // ==========================
    if (page === "sell.html") {
    ...
    if (page === "profile.html") {
        ...
        async function loadProfileData() {
            ...
            avatarInput?.addEventListener("change", async (e) => {
                ...
                try {
                    ...
                    localStorage.setItem(`avatar_${session.user.id}`, avatarUrl);
        ...
        window.deleteItem = async function(id) {
            if (!confirm("Are you sure you want to remove this listing from Campus Market?")) return;
            ...
            if (error) {
                if (profileMsg) profileMsg.innerText = "❌ Error deleting listing: " + error.message;
            } else {
    ...
    // ==========================
    // AUTHENTICATION & LOGOUT (ENHANCED)
    // ==========================
    window.signup = async function () {
        ...
        if (data.user && data.session === null) {
            ...
            msgElement.innerText = "✅ Account created! Please check your email inbox to verify your account, then log in.";
        } else {
            msgElement.style.color = "var(--accent-emerald)";
